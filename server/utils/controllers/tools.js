...
const toolTemplate = require('../templates/tool');
const dataUtil = require('../dataManipulation');
const config = require('../../config/config.json');
const Op = models.Sequelize.Op;

//
//	Get tools taxonomies
//
exports.taxs = async function(resource, type = 'TOOLS'){
	let includes = [
		{
			model: models.Term,
...
exports.notifyNew = function(data){
  ...
  .then(function(users){
    if (users && users.length>0){
      ...
      // Notify
      const templateFunction = toolTemplate.newWithValidator;

      // Notify Users to email
      dataUtil.sendEmail({
        sender: config.teamName+" <"+config.teamEmail+">",
        to: data.user.name  + " <"+data.user.email+">",
        subject: "REDA - Nova ferramenta",
  ...
  .then(function(users){
    if (users && users.length>0){
      ...
      dataUtil.sendEmail({
        sender: config.teamName+" <"+config.teamEmail+">",
        to: dests,
        subject: "REDA - Ferramenta atualizada",
        html: toolTemplate.update({
          "username": data.user.name,
          "slug": data.slug,
          "title": data.title
        })
...
exports.notifyStatus = function(data){
  dataUtil.sendEmail({
    ...
    html: toolTemplate.notifyStatus({
