...
export default class AppsOverview extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return this._isMounted && (nextProps.apps.fetched || nextState.update===true);
  }
  ...
  requestData(reset = false, state = null){
    if(this._isMounted){
      ...
      return this.props.searchMonthApps(initialData);
    }
  }
  ...
  render() {
    ...
    return (
      <section className="container-fluid">
        <h1 className="margin__bottom--30">Novas aplicações do mês</h1>
        <div className="row">
          {apps.fetched && apps.data && apps.data.length>0 ?
